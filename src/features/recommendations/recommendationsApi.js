...
export const getRecommendedProjects = async (limit = 10) => {
  const data = await apiClient.get('/v1/recommendations/projects', {
    params: { limit },
  });
  return data;
};
...
export const getRecommendedProfessors = async (projectTemplateId = '', limit = 10) => {
  const data = await apiClient.get('/v1/recommendations/professors', {
 ...
 * Maps to GET /api/v1/recommendations/skills
 * 
 * @param {number} [limit=10] - Optional max count of skill recommendations to return (default: 10).
 * @returns {Promise<RecommendedSkillDto[]>} Array of pre-ranked skill growth recommendations.
 */
export const getRecommendedSkills = async (limit = 10) => {
  const data = await apiClient.get('/v1/recommendations/skills', {
    params: { limit },
  });
  return data;
};